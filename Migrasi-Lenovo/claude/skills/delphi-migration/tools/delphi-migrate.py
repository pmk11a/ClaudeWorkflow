...
try:
    from pas_parser import EnhancedPASParser, print_analysis
    from dfm_parser import EnhancedDFMParser, print_analysis as print_dfm_analysis
...
try:
    ...
    from controller_generator import EnhancedControllerGenerator, create_from_parser_result as create_controller_generator
    ...
    from view_generator import EnhancedViewGenerator
except ImportError as e:
...
class DelphiMigrationCLI:
    ...
    def migrate(self, pas_file: str, dfm_file: str = None, model_name: str = None) -> Dict[str, Any]:
        ...
        # 8. Generate Sidebar Menu Snippet
        print_step("Generating Sidebar Menu Snippet...")
        try:
            snippet_content, snippet_filename = self.generate_sidebar_snippet(model_name, module_name)
            snippet_file = self.output_dir / snippet_filename

            if not self.config.dry_run:
                snippet_file.write_text(snippet_content)
                print_file(snippet_filename)
                self.stats['files_generated'] += 1
                generated_files.append(str(snippet_file))
                print_info(f"📋 Copy content from {snippet_filename} to resources/views/layouts/app.blade.php")
                print_info("   Don't forget to run: php artisan cache:clear && php artisan view:clear")
            else:
                print_file(f"{snippet_filename} (dry run)", 'skipped')
        except Exception as e:
            raise GenerationError("Sidebar snippet", str(e))
    ...
    def generate_sidebar_snippet(self, model_name: str, module_name: str, display_name: str = None) -> Tuple[str, str]:
        """
        Generate sidebar menu HTML snippet

        Args:
            model_name: Model name (e.g., 'PPL')
            module_name: Module name in lowercase (e.g., 'ppl')
            display_name: Display name for menu (e.g., 'Penyerahan Bahan')

        Returns:
            Tuple of (snippet_content, filename)
        """
        if not display_name:
            # Auto-generate from model_name
            display_name = model_name.replace('_', ' ')

        snippet = f'''<!-- {model_name} Menu Item -->
<!-- Add this to resources/views/layouts/app.blade.php in the appropriate section -->
...
<li class="nav-item">
    ...
    <ul class="nav nav-treeview">
        <li class="nav-item">
            <a href="{{{{ route('{module_name}.index') }}}}" class="nav-link">
                <i class="far fa-circle nav-icon"></i>
                <p>Daftar {display_name}</p>
            </a>
        </li>
        <li class="nav-item">
            <a href="{{{{ route('{module_name}.create') }}}}" class="nav-link">
                <i class="far fa-circle nav-icon"></i>
                <p>Tambah {display_name}</p>
            </a>
        </li>
    </ul>
</li>
...
<!--
    ...
    2. Find the appropriate section:
       - Master Data → Look for "Master Data" menu section
       - Transaksi → Look for "Transaksi" menu section
       - Laporan → Look for "Laporan" menu section

    3. Copy the menu item above and paste it in the correct section

    4. CRITICAL: Clear all caches after adding
    ...
    TROUBLESHOOTING:
    - Menu not showing → Clear cache again
    - 404 error → Check route names match
    - Access denied → Check policy in {model_name}Policy.php
    - Icons not showing → Verify Font Awesome included in layout
-->
'''

        filename = f"sidebar_{module_name}.html"
